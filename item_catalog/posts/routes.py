...
def newItem():
    if request.method == 'POST':
        ...
        return redirect(url_for('main.showAll'))
    else:
...
def deleteItem(item_id):
    itemDelete = session.query(Item).filter_by(id=item_id).first()
    if request.method == 'POST':
        session.delete(itemDelete)
        session.commit()
        return redirect(url_for('main.showAll', item_id = item_id))
    else:
...
def newCategory(item_id):
    itemName = session.query(Item).filter_by(id=item_id).first()
...
def editDetails(item_id, itemDetails_id):
    itemName = session.query(Item).filter_by(id=item_id).first()
    editedItem = session.query(ItemCategory).filter_by(id=item_id).first()
    if request.method == 'POST':
        if request.form['name']:
...
def deleteDetails(item_id, itemDetails_id):
    itemToDelete = session.query(ItemCategory).filter_by(id=item_id).first()
    item = session.query(Item).filter_by(id=item_id).first()
    if request.method == 'POST':
